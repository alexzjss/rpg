...
export default async function handler(req: any, res: any) {
  try {
    ...
    const action = data.grimoire.find((item: any) => item.id === input.actionId) ?? data.abilityGraphs?.find((item: any) => item.id === input.actionId);
    ...
    const isReaction = (actionInfo.tags ?? []).some((tag: string) => tag.toLocaleLowerCase('pt-BR').includes('reação') || tag.toLowerCase().includes('reacao'));
    ...
    const target = actionInfo.target ?? { type: 'um_alvo' };
